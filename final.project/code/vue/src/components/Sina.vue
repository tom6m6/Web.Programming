...
export default {
  ...
  methods: {
    ...
    save(){
      this.request.post('/sina',this.form).then(res => {
        if(res.code === 200)
    ...
    del(id){
      this.request.delete('/sina',{data: {id}}).then(res => {
        if(res.code === 200)
    ...
    batchDelete(){
      ...
      this.request.delete('/sina/batch',{data: ids}).then(
          res => {
            if(res.code === 200)
              this.$message.success("删除成功");
            else
    ...
    updateNews(){
      this.request.get('/sina/update').then(
        res => {
          if(res.code === 200)
