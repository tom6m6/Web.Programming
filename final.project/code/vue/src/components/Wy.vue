<template>
  <div>
    <div style="margin: 10px 0">
      ...
      <el-input style="width: 200px" v-model="searchKeywords" placeholder="请输入关键词" suffix-icon="el-icon-search" class="ml-5"></el-input>
    ...
    <el-table :data="tableData" border stripe @selection-change="batchSel">
      ...
      <el-table-column prop="keywords" label="关键词" width="150"></el-table-column>
      <el-table-column prop="source" label="来源" width="150"></el-table-column>
      <el-table-column prop="comment_total" label="评论数" width="150"></el-table-column>
    ...
    <el-dialog title="新闻信息" :visible.sync="dialogFormVisible" width="30%" >
      <el-form label-width="80px" size="small">
        ...
        <el-form-item label="评论数">
          <el-input v-model="form.comment_total" autocomplete="off"></el-input>
        </el-form-item>
...
methods: {
  ...
  save(){
    this.request.post('/wy',this.form).then(res => {
      if(res.code === 200)
  ...
  del(id){
    this.request.delete('/wy',{data: {id}}).then(res => {
      if(res.code === 200)
  ...
  batchDelete(){
    ...
    this.request.delete('/wy/batch',{data: ids}).then(
        res => {
          if(res.code === 200)
            this.$message.success("删除成功");
          else
  ...
  updateNews(){
    this.request.get('/wy/update').then(
      res => {
        if(res.code === 200)
